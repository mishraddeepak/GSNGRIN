...
export default function Attendee() {
    ...
    const gsndate = location.state?.gsnDate || '';
    const initialgsnDate = new Date(gsndate).toISOString().split('T')[0];

    const [grinNo, setGrinNo] = useState("");
    const [grinDate, setGrinDate] = useState("");
    const [gsn, setGsn] = useState(initialselectedValue);
...
console.log(initialselectedValue,"&",initialgsnDate)
